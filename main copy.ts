...
export default class DiaryMergerPlugin extends Plugin {
    ...
    async onload() {
        ...
        // 添加侧边栏按钮
        this.addRibbonIcon('files', 'Merge Diary Entries', () => {
            this.mergeDiaries();
        });
    ...
    async autoMergeIfNeeded() {
        ...
        if (files.length >= this.settings.maxEntriesPerFile) {
            new Notice('Auto-merging diary entries...');
    ...
    async mergeDiaries() {
        ...
        if (files.length === 0) {
            new Notice('No diary files to merge');
            return;
        }
    ...
    async getDiaryFiles(): Promise<TFile[]> {
        ...
        if (!(diaryFolder instanceof TFolder)) {
            new Notice(`Diary folder not found: ${this.settings.diaryFolder}`);
            return [];
        }
    ...
    async performMerge() {
        ...
        // 获取或创建合并文件
        const mergedFile = await this.getOrCreateMergedFile();
        ...
        new Notice(`已合并 ${files.length} 篇日记到 ${mergedFile.name}`);
    }
    ...
    // 获取或创建合并文件
    async getOrCreateMergedFile(): Promise<TFile> {
        const diaryFolder = this.app.vault.getAbstractFileByPath(this.settings.diaryFolder);
        if (!(diaryFolder instanceof TFolder)) {
            throw new Error('日记文件夹不存在');
        }
        
        // 查找现有合并文件
        const existingFiles = diaryFolder.children
            .filter((file): file is TFile => file instanceof TFile)
            .filter(file => file.name.startsWith('merged-'));
        
        // 追加模式且存在合并文件
        if (this.settings.mergeStrategy === 'append' && existingFiles.length > 0) {
            return existingFiles[0];
        }
        
        // 创建新合并文件
        const timestamp = window.moment().format('YYYYMMDDHHmmss');
        const newFileName = `merged-${timestamp}.md`;
        const newFilePath = `${this.settings.diaryFolder}/${newFileName}`;
        
        return this.app.vault.create(newFilePath, '');
    }
...
class DiaryMergeSettingTab extends PluginSettingTab {
    ...
    display(): void {
        ...
        new Setting(containerEl)
            ...
            .addText(text => text
                .setPlaceholder('例如: journal/daily')
        ...
        new Setting(containerEl)
            ...
            .setDesc('备份日记的文件夹路径')
            .addText(text => text
                .setPlaceholder('例如: journal/backups')
